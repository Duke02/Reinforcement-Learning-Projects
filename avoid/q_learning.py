...
class QLearningAgent:
    ...
    def get_action(self, obs: tp.Dict[str, np.ndarray]) -> int:
        """
    ...
    def update(
        self,
        obs: tp.Dict[str, np.array],
        action: int,
        reward: float,
        terminated: bool,
        next_obs: tp.Dict[str, np.ndarray],
    ):
        """Updates the Q-value of an action."""
        future_q_value = (not terminated) * np.max(self.q_values[next_obs.values()])
        temporal_difference = reward + self.discount_factor * future_q_value - self.q_values[obs.values()][action]

        # Update the current experience we have with the environment by a small nudge towards what we think is the
        # optimal action.
        self.q_values[obs.values()][action] = self.q_values[obs.values()][action] + self.lr * temporal_difference
